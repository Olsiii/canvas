...
test("M4.3 chat: create a channel, thread a reply, see a teammate's message live, and ask Brain", async ({
  ...
  // Thread a reply on Ada's own message. The reply composer renders right
  // after the message it's replying to, ahead of the channel's main
  // composer at the bottom — so it's the *first* "message-composer" in DOM
  // order, not the last.
  await page.getByTestId("channel-messages").getByText("Hello team").hover();
  await page.getByRole("button", { name: "Reply" }).click();
  const replyComposer = page.locator('[data-testid="message-composer"]').first();
  await replyComposer.click();
  await page.keyboard.type("On it!");
  await page.locator('[data-testid="message-send"]').first().click();
